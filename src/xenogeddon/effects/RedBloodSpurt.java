...
public final class RedBloodSpurt extends AbstractEffect {

	private static final long DURATION = 300;

	private ParticleMesh mesh;
	private boolean active = false;
	private static ArrayList<RedBloodSpurt> spurts = new ArrayList<RedBloodSpurt>();

	public static void InitBloodSpurt(Main m) {
		for (int i=0 ; i<10 ; i++) {
			RedBloodSpurt spark = new RedBloodSpurt(m);
			spurts.add(spark);
	...
	public static void AddBloodSpurt(Main main, Vector3f pos, Vector3f dir) {
		for (int i=0 ; i<spurts.size() ; i++) {
			RedBloodSpurt s = (RedBloodSpurt)spurts.get(i);
			if (s.active == false) {
				//Main.p("Blood " + i);
				s.active = true;
	...
	private RedBloodSpurt(Main m) {
		super(m, "BloodSpurt", 0f, 0f, 0f, false, -1);
		...
		mesh.setEndSize(.1f);
		mesh.setStartColor( new ColorRGBA( 1.0f, 0.0f, 0.0f, 1.0f ) );
		mesh.setEndColor( new ColorRGBA( 1.0f, 0.0f, 0.0f, 0.0f ) );
