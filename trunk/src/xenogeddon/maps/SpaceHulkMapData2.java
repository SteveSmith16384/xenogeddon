...
public class SpaceHulkMapData2 extends AbstractMapData {

	private static final int MAX = 40;
	private static final int MAX_ROOM_SIZE = 5;
	private static final int ROOMS = 9;

	public SpaceHulkMapData2() {
		...
		while (true) {
			...
			for (int r=0 ; r<ROOMS ; r++) {
				...
				if (r == 0) { // Start room
					...
					player_start_pos = new Point(x, y);
					map[x+1][y+1].blocking_scenery_id = MapSquare.TELEPORTER;
					//if (!Main.RELEASE_MODE) {
					this.addGoodyStartPos(x+1, y+2);
					this.addGoodyStartPos(x+2, y+1);
					//}
			...
			for (int i=0 ; i<centres.size() ; i++) {
				...
				super.addPipes(start.x+3, end.y+3, Functions.rnd(0,1) == 0);
