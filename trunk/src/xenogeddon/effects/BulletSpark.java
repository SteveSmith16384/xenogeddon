...
public final class BulletSpark extends AbstractEffect {

	private static final long DURATION = 200;

	private ParticleMesh mesh;
	private boolean active = false;
	private static ArrayList<BulletSpark> sparks = new ArrayList<BulletSpark>();

	public static void InitBulletSpark(Main m) {
		for (int i=0 ; i<10 ; i++) {
			BulletSpark spark = new BulletSpark(m);
			sparks.add(spark);
	...
	public static void AddBulletSpark(Main main, Vector3f pos, Vector3f dir) {
		for (int i=0 ; i<sparks.size() ; i++) {
			BulletSpark s = (BulletSpark)sparks.get(i);
			if (s.active == false) {
				//Main.p("Found " + i);
				s.active = true;
	...
	private BulletSpark(Main m) {
		super(m, "BulletSpark", 0f, 0f, 0f, false, -1);
